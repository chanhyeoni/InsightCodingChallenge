/*
 ...
 Insight Coding Challenge by the Insight Coding Challenge
...
public class Main {
    ...
    // the node class that is designed to store the degree of the node as well as its adjacent node
    // each adjacent node contains
    public class Node{
        double degree;
    ...
    void Remove(HashMap<String, Node> graph, Date leastPaymentDateLimit){
        // the function that removes the node from the graph by comparing the createdTime to the minimum timestamp
    ...
    void Add(HashMap<String, Node> graph, String actor, String target, Date createdTime){
        // the function that takes the graph object as well as the new data entry (actor, target, createdTime)
        // and adds the entry to the graph
        
        // check if the graph contains actor
        if (graph.containsKey(actor)){
            // if actor is already present, add this to the list of the adjacent nodes
            Pair<Date, String> obj = new Pair(createdTime, target);
            graph.get(actor).adjacentNodes.add(obj);
            graph.get(actor).degree++;
        }else{
            // if not, create a new adjancency list and add the entry
    ...
    double getMedian(HashMap<String, Node> graph){
        // the function that returns the median value of the graph
        // first, get all the degrees of each node into an array
        // second, sort the data of the array
        // then, by the rule to get the median, estimate it.
        // (if the length of the array is odd, return the middle one; otherwise, return the average of the two middle ones of the array)
        ...
        if (degreesArray.length % 2 == 0) {
            median = Math.floor((((double)degreesArray[degreesArray.length / 2] + (double)degreesArray[degreesArray.length / 2 - 1]) / 2)*100)/100;
        }else {
            median = Math.floor((double)degreesArray[degreesArray.length / 2]*100)/100;
        }
